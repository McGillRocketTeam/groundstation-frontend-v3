...
export class WebSocketClient {
  ...
  private subscriptionCache: Map<
    string,
    Map<string, WebSocketCall<unknown, (data: unknown) => void>>
  > = new Map();

  constructor(private frameLossListener: FrameLossListener) {
    const currentLocation = window.location;
    // const url = "ws://localhost:8090/yamcs/api/websocket";
    let url = "ws://";
    if (currentLocation.protocol === "https:") {
      url = "wss://";
    }
    url += `${currentLocation.hostname}:8090/yamcs/api/websocket`;
  ...
  createSubscription<O extends Object, D>(
    ...
    if (!this.subscriptionCache.has(type)) {
      this.subscriptionCache.set(type, new Map());
    }
    const subTypeCache = this.subscriptionCache.get(type)!;
    const hashStr = hash(options);
    const existingWebSocketSub = subTypeCache.get(hashStr);
    if (
      existingWebSocketSub !== undefined &&
      !existingWebSocketSub.isCanceled
    ) {
      // @ts-ignore
      existingWebSocketSub.addMessageListener(observer);
      // @ts-ignore
    ...
    // @ts-ignore
    subTypeCache.set(hashStr, websocketSub);
    return websocketSub;
  }
